...
describe('GitHub plugin sources', () => {
  it('normalizes URL and shorthand forms and rejects unsafe URLs and refs', () => {
    expect(parseGitHubPluginSpecifier('https://github.com/Deep-TUI/Concise.git#v1.2.0')).toMatchObject({
      owner: 'deep-tui', repository: 'concise', ref: 'v1.2.0',
      specifier: 'https://github.com/deep-tui/concise#v1.2.0',
    })
    expect(parseGitHubPluginSpecifier('github:deep-tui/concise')).toMatchObject({
      owner: 'deep-tui', repository: 'concise',
    })
    expect(parseGitHubPluginSpecifier('@deep-tui/plugin-example')).toBeUndefined()
    expect(() => parseGitHubPluginSpecifier('https://token@github.com/deep-tui/concise')).toThrow(/uncredentialed HTTPS/)
    expect(() => parseGitHubPluginSpecifier('https://github.com/deep-tui/concise/tree/main')).toThrow(/one repository/)
    expect(() => parseGitHubPluginSpecifier('github:deep-tui/concise#../unsafe')).toThrow(/invalid GitHub plugin ref/)
  })

  it('installs once, uses the cache, and atomically refreshes', async () => {
    const root = await temporaryDirectory('deep-tui-github-cache-')
    const commands: Array<{ command: string; args: string[]; cwd?: string }> = []
    const specifier = 'https://github.com/deep-tui/concise#v1'
  ...
  it('adds and removes a normalized GitHub URL without treating it as an npm package', async () => {
    const directory = await temporaryDirectory('deep-tui-remote-config-')
    const config = path.join(directory, 'deep-tui.config.json')
    await writeFile(config, JSON.stringify({ version: 2, plugins: [] }))

    const added = await addPlugin(directory, 'github:Deep-TUI/Concise.git#stable', { install: false })
    expect(added.entry.use).toBe('https://github.com/deep-tui/concise#stable')
    expect((await readConfig(config)).plugins).toEqual([
      { use: 'https://github.com/deep-tui/concise#stable' },
    ])

    await removePlugin(directory, 'github:deep-tui/concise#stable')
    expect((await readConfig(config)).plugins).toEqual([])
  })

  it('loads a cached GitHub plugin directly from composition', async () => {
    const directory = await temporaryDirectory('deep-tui-remote-composition-')
    const configDirectory = path.join(directory, 'config')
    const workspace = path.join(directory, 'workspace')
    const root = path.join(directory, 'managed')
    const config = path.join(configDirectory, 'deep-tui.config.json')
    ...
    await writeFile(config, JSON.stringify({
      ...
      plugins: [
        ...
        { use: 'https://github.com/deep-tui/remote-command#stable' },
      ],
    }))
