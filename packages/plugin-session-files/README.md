# @deep-tui/plugin-session-files

Persists canonical conversations beneath `.deep-tui/sessions` and contributes
`/new`, `/sessions`, `/resume`, `/fork`, `/rename`, and `/session`.

The store uses versioned JSONL, optimistic sequence checks, atomic indexes, and
cross-process lock diagnostics. `deep-tui sessions` supports list, show, export,
