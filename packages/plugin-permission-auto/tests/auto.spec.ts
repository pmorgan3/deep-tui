...
import { PermissionService, TuiService, type TuiActions } from '@flect/sdk'
...
describe('auto permission mode', () => {
  it('starts off, toggles through /auto, excludes network, and does not persist a rule', async () => {
    ...
    await expect(ctx.permissions.authorize(request('read'))).resolves.toMatchObject({ decision: 'allow', policyId: 'flect.permission.auto' })
